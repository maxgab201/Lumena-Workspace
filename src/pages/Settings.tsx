...
export const Settings = () => {
  ...
  const tabs = [
    ...
    { id: 'appearance', label: t('settings.appearance'), icon: <Monitor size={16} /> },
  ...
  return (
    <PageContainer>
      <header className="space-y-1 mb-8">
        <h1 className="text-3xl font-heading font-semibold tracking-tight">{t('settings.title')}</h1>
            ...
            >
              {activeTab === 'profile' && (
                <Card className="border-white/5 bg-card/40 backdrop-blur-md">
                  <CardHeader>
                    <CardTitle className="text-xl">{t('settings.profileInfo')}</CardTitle>
                  ...
                  <CardContent className="space-y-6">
                    ...
                    <div className="space-y-4">
                      <div className="grid gap-1.5">
                        <label htmlFor="name-input" className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t('settings.name')}</label>
                        <input id="name-input" type="text" value={name} onChange={(e) => setName(e.target.value)} className="h-10 w-full rounded-lg border border-white/10 bg-background/30 backdrop-blur-sm px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-accent" />
                      </div>
                      <div className="grid gap-1.5">
                        <label htmlFor="email-input" className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t('settings.email')}</label>
                        <input id="email-input" type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="h-10 w-full rounded-lg border border-white/10 bg-background/30 backdrop-blur-sm px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-accent" />
                      </div>
                    </div>
              ...
              {activeTab === 'appearance' && (
                <Card className="border-white/5 bg-card/40 backdrop-blur-md">
                  <CardHeader>
                    <CardTitle className="text-xl">{t('settings.themeAppearance')}</CardTitle>
                    <CardDescription>{t('settings.themeAppearanceDesc')}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    <div className="space-y-3">
                      <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t('settings.theme')}</label>
                      <div className="grid grid-cols-3 gap-3">
                        {[
                          { id: 'light', label: t('settings.light'), icon: <Sun size={18} /> },
                          { id: 'dark', label: t('settings.dark'), icon: <Moon size={18} /> },
                          { id: 'system', label: t('settings.system'), icon: <Monitor size={18} /> },
                        ].map((item) => (
                          <button key={item.id} onClick={() => setTheme(item.id as 'light' | 'dark' | 'system')} className={`flex flex-col items-center justify-center p-4 rounded-xl border transition-all duration-200 gap-2 focus:outline-none focus:ring-2 focus:ring-accent ${theme === item.id ? 'border-accent bg-accent/5 text-foreground' : 'border-white/5 bg-secondary/20 text-muted-foreground hover:text-foreground hover:bg-secondary/40'}`}>
                            {item.icon}
                    ...
                    <div className="pt-6 border-t border-white/5 space-y-4">
                      <h4 className="text-sm font-semibold">{t('settings.workspaceLayout')}</h4>
                      <div className="flex items-center justify-between p-3 rounded-lg bg-secondary/10 border border-white/5">
                        <div>
                          <p className="text-xs font-semibold text-foreground">{t('settings.compactSidebar')}</p>
                          <p className="text-[11px] text-muted-foreground">{t('settings.compactSidebarDesc')}</p>
                        </div>
              ...
              {activeTab === 'notifications' && (
                <Card className="border-white/5 bg-card/40 backdrop-blur-md">
                  <CardHeader>
                    <CardTitle className="text-xl">{t('settings.notificationPrefs')}</CardTitle>
              ...
              {activeTab === 'shortcuts' && (
                <Card className="border-white/5 bg-card/40 backdrop-blur-md">
                  <CardHeader>
                    <CardTitle className="text-xl">{t('settings.keyboardShortcuts')}</CardTitle>
                    <CardDescription>{t('settings.keyboardShortcutsDesc')}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-6">
                            ...
                            {grp.keys.map((sh, k) => (
                              <div key={k} className="flex items-center justify-between text-xs font-medium">
                                ...
                                <div className="flex items-center space-x-1">
                                  {sh.keys.map((key, ki) => (
                                    <kbd key={ki} className="inline-flex items-center justify-center px-2 py-0.5 rounded border border-white/10 bg-background/50 font-mono text-[10px] text-foreground shadow-sm">{key}</kbd>
                                  ))}
                                </div>
                              </div>
                            ))}
              ...
              {activeTab === 'about' && (
                <Card className="border-white/5 bg-card/40 backdrop-blur-md">
                  <CardHeader>
                    <CardTitle className="text-xl">{t('settings.aboutLumena')}</CardTitle>
                    <CardDescription>{t('settings.aboutLumenaDesc')}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-6">
